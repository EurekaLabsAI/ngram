...
seq_len = st.sidebar.slider("Initial Sequence Length (N-gram)", 2, 5, 4)
...
st.subheader("🎨 Hyperparameter Tuning with Animated Training and Validation Loss")
seq_lens = [3, 4]
smoothings = [0.03, 0.1]
best_loss = float('inf')
best_params = {'seq_len': None, 'smoothing': None}
loss_fig, loss_ax = plt.subplots()
loss_plot = st.pyplot(loss_fig)
training_losses, validation_losses = [], []

# Hyperparameter combinations
for trial_idx, (seq_len_trial, smoothing_trial) in enumerate(itertools.product(seq_lens, smoothings), 1):
    ...
    training_losses.append((trial_idx, train_loss))
    validation_losses.append((trial_idx, val_loss))

    loss_ax.clear()
    loss_ax.plot([x[0] for x in training_losses], [x[1] for x in training_losses], 'g-', label='Training Loss')
    loss_ax.plot([x[0] for x in validation_losses], [x[1] for x in validation_losses], 'b-', label='Validation Loss')
    loss_ax.set_title(f"Hyperparameter Tuning - Trial {trial_idx}")
    loss_ax.set_xlabel("Trial Index")
    loss_ax.set_ylabel("Loss")
    loss_ax.legend()
    # write ylim here to avoid resetting ylim
    loss_ax.set_ylim(0, 4)
    loss_plot.pyplot(loss_fig)
...
st.write(f"Training Loss with Best Parameters: {train_loss:.4f}")
